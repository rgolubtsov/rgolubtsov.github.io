# Installed packages on OpenBSD 7.7 VM box

### OpenBSD/amd64 7.7 `$ pkg_info` at `radicv760.my.domain`

The following packages have been installed/upgraded as of 19th of May, 2025:
...
chromium-135.0.7049.52p0 Chromium browser
...
curl-8.13.0         transfer files with FTP, HTTP, HTTPS, etc.
...
erlang-26.2.5.11v0  real-time, concurrent and distributed functional language
erlang-27.3.3v0     real-time, concurrent and distributed functional language
...
gitwrapper-0.110p0  invoke an appropriate Git repository server
...
gtk4-update-icon-cache-4.18.3 gtk+ icon theme caching utility
harfbuzz-11.0.0     text shaping library
...
libxkbcommon-1.8.1  library to handle keyboard descriptions
libxml-2.13.8       XML parsing library
libxslt-1.1.43p0    XSLT C Library for GNOME
libyaml-0.2.5       YAML 1.1 parser and emitter written in C
llvm-17.0.6p12      modular, fast C/C++/ObjC compiler, static analyzer and tools
...
nghttp2-1.65.0      library for HTTP/2
...
node-22.15.1v0      JavaScript runtime built on Chrome's V8 JavaScript engine
...
nss-3.110           libraries to support development of security-enabled apps
...
openssl-3.3.3p0v0   TLS/SSL library and tools
...
[OpenBSD](https://openbsd.org) 7.7 ~~has been upgraded~~ to the [7.8](/data/blog/pkg_info/78) release.
