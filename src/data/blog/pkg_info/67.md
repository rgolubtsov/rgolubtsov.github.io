# Installed packages on OpenBSD 6.7 VM box

### OpenBSD/amd64 6.7 `$ pkg_info` at `radicv610.my.domain`

The following packages have been installed or upgraded as of 1st of June, 2020:

```
$ pkg_info
adwaita-icon-theme-3.34.3 base icon theme for GNOME
aom-1.0.0.20200402  Alliance for Open Media AV1 video codec
at-spi2-atk-2.34.2  atk-bridge for at-spi2
at-spi2-core-2.34.0p0 service interface for assistive technologies
atk-2.34.1p1        accessibility toolkit used by gtk+
avahi-0.8           framework for Multicast DNS Service Discovery
bash-5.0.17         GNU Bourne Again Shell
brotli-1.0.7        generic lossless compressor
bzip2-1.0.8         block-sorting file compressor, unencumbered
cairo-1.16.0        vector graphics library
cdparanoia-3.a9.8p4 CDDA reading utility with extra data verification features
chromium-81.0.4044.138 Chromium browser
clojure-1.10.1.469  Lisp-1 dialect for the Java Virtual Machine
cmake-3.16.2p1v0    portable build system
colord-1.3.5p1      device color profile management daemon
colordiff-1.0.18    colorized diff tool
consolekit2-1.2.1p8 framework for defining and tracking users, sessions & seats
cups-libs-2.3.3     CUPS libraries and headers
curl-7.69.1         get files from FTP, Gopher, HTTP or HTTPS servers
cvsps-2.1p2         generate patchsets from CVS repositories
dav1d-0.6.0         small and fast AV1 decoder
dbus-1.12.16p0v0    message bus system
dbus-daemon-launch-helper-1.12.16p0 DBus setuid helper for starting system services
dconf-0.34.0p0      configuration backend system
desktop-file-utils-0.24p0 utilities for dot.desktop entries
dos2unix-7.4.1      convert DOS/MAC files to UNIX (line-endings/charset)
e2fsprogs-1.42.12p5 utilities to manipulate ext2 filesystems
elixir-1.10.2       functional language for building scalable applications
enca-1.19p0         detect character set and encoding of text files
erlang-19.3p7v0     real-time, concurrent and distributed functional language
erlang-21.2p3v0     real-time, concurrent and distributed functional language
ffmpeg-4.2.2v0      audio/video converter and streamer
flac-1.3.3          free lossless audio codec
flock-20110525p1    flock(2) manager for shell scripts
fribidi-1.0.9       library implementing the Unicode Bidirectional Algorithm
gcc-8.3.0p5         GNU compiler collection: core C compiler
gcc-libs-8.3.0p5    GNU compiler collection: support libs
gdbm-1.18.1p0       GNU dbm
gdk-pixbuf-2.40.0p2 graphic library for GTK+
gettext-runtime-0.20.1p1 GNU gettext runtime libraries and programs
giflib-5.1.6        tools and library routines for working with GIF images
git-2.26.2p0        distributed version control system
glib2-2.62.6        general-purpose utility library
glib2-networking-2.62.4 network-related gio modules for GLib
gmake-4.3           GNU make
gmp-6.2.0           library for arbitrary precision arithmetic
gnome-icon-theme-3.12.0p5 base icon theme for GNOME
gnome-icon-theme-symbolic-3.12.0p3 base icon theme extension for special UI contexts
gnutls-3.6.13       GNU Transport Layer Security library
go-1.13.9           Go programming language
graphite2-1.3.14    rendering for complex writing systems
gsettings-desktop-schemas-3.34.0 collection of shared GSettings schemas
gsm-1.0.19          GSM audio codec library and converter
gtk+2-2.24.32p9     multi-platform graphical toolkit
gtk+3-3.24.20       multi-platform graphical toolkit
gtk+3-cups-3.24.20  gtk+3 CUPS print backend
gtk-update-icon-cache-3.24.20 gtk+ icon theme caching utility
harfbuzz-2.6.5      text shaping library
hicolor-icon-theme-0.17 fallback theme of the icon theme specification
htop-2.2.0p14       interactive process viewer
icu4c-67.1          International Components for Unicode
intel-firmware-20190918v0 microcode update binaries for Intel CPUs
jansson-2.12        library for manipulating JSON data
jasper-2.0.14       reference implementation of JPEG-2000
javaPathHelper-2.0  helper script for launching java applications
jdk-1.8.0.252.b09.1v0 OpenJDK Software Development Kit v1.8.0.252.b09.1
jpeg-2.0.4p0v0      SIMD-accelerated JPEG codec replacement of libjpeg
json-glib-1.4.4     JSON parser for GLib-based libraries and applications
jsoncpp-1.8.4p2     JSON parsing C++ API
lame-3.100p1        lame ain't an MP3 encoder
lcms-1.19           color management library
lcms2-2.9p0         color management library
libao-1.2.0p1       portable audio output library
libarchive-3.4.2    multi-format archive and compression library
libass-0.14.0p0     portable ASS/SSA subtitle renderer
libb2-0.98.1v0      library providing BLAKE2b, BLAKE2s, BLAKE2bp, BLAKE2sp
libbluray-1.1.2     library supporting Blu-ray playback
libbs2b-3.1.0p3     Bauer stereophonic-to-binaural DSP
libcroco-0.6.13     generic CSS parsing library for GNOME project
libdaemon-0.14p1    lightweight C library that eases the writing of daemons
libdockapp-0.4.0p5  dockapp-making standard library for Window Maker
libdv-1.0.0p3       Quasar DV codec
libdvdcss-1.4.2p1   descramble scrambled DVDs
libdvdnav-6.0.1v0   DVD navigation library
libdvdread-6.0.2    accessing DVD files
liberation-fonts-2.00.1p1 substitute for MS TTF core fonts
libev-4.27          high-performance event loop library
libevent-2.1.11     event notification library
libexecinfo-0.3p2v0 clone of backtrace facility found in the GNU libc
libexif-0.6.22pre20200229 extract digital camera info tags from JPEG images
libffi-3.3          Foreign Function Interface
libgcrypt-1.8.5     crypto library based on code used in GnuPG
libgnome-keyring-3.12.0p6 library for gnome keyring integration
libgpg-error-1.37   error codes for GnuPG related software
libiconv-1.16p0     character set conversion library
libid3tag-0.15.1bp5 library for reading ID3 tags
libidn2-2.3.0p0     implementation of IDNA2008 internationalized domain names
libmad-0.15.1bp6    high-quality MPEG audio decoder
libmicrohttpd-0.9.70 tiny embeddable httpd library written in C
libmng-1.0.10p3     Multiple-image Network Graphics (MNG) reference library
libmpc-1.1.0        arbitrary-precision complex numbers math library
libnettle-3.5.1p0   cryptographic library
libogg-1.3.4        Ogg bitstream library
libproxy-0.4.15p4   library handling all the details of proxy configuration
libpsl-0.20.2p1     public suffix list library
librsvg-2.48.4      SAX-based render library for SVG files
libsamplerate-0.1.9 audio sample rate conversion library
libsndfile-1.0.28   library to handle various audio file formats
libsoup-2.68.4      HTTP client/server library for GNOME
libtasn1-4.16.0     Abstract Syntax Notation One structure parser library
libtheora-1.2.20190601p0 open video codec
libunbound-1.10.1   validating DNS resolver library
libunistring-0.9.7  manipulate Unicode strings
libuv-1.30.1        multi-platform library for asynchronous I/O
libv4l-1.18.0       libv4l userspace library
libvorbis-1.3.6     audio compression codec library
libvpx-1.8.2v0      Google VP8/VP9 video codec
libxml-2.9.10p0     XML parsing library
libxslt-1.1.34      XSLT C Library for GNOME
lua-5.2.4p1         powerful, light-weight programming language (version 5.2.4)
lynx-2.8.9rel1p0    text web browser
lz4-1.9.2p0         fast BSD-licensed data compression
lzo2-2.10p2         portable speedy lossless data compression library
mariadb-client-10.4.12v1 multithreaded SQL database (client)
maven-3.6.0p0       software project management and comprehension tool
mpfr-4.0.2.1        library for multiple-precision floating-point computations
mpg321-0.3.2p1      free clone of mpg123, a command-line mp3 player
mplayer-20200229    movie player supporting many formats
nghttp2-1.40.0      library for HTTP/2
nginx-1.16.1p1      robust and small HTTP server and mail proxy server
node-12.16.1        V8 JavaScript for clients and servers
noto-emoji-20180810 emoji fonts for the noto font family
noto-fonts-20171024 pan-unicode font family
nspr-4.25           Netscape Portable Runtime
nss-3.51.1          libraries to support development of security-enabled apps
opus-1.3.1          IETF audio codec
p11-kit-0.23.20     library for loading and enumerating PKCS#11 modules
p5-Canary-Stability-2013 canary to check perl compatibility for schmorp's modules
p5-Digest-HMAC-1.03p0 interface to HMAC Message-Digest Algorithms
p5-EV-4.27          perl interface to libev
p5-Error-0.17028    error/exception handling in an OO-ish way
p5-IO-Socket-SSL-2.068 Perl SSL sockets with IO::Socket interface
p5-Mail-Tools-2.21  modules for handling mail with perl
p5-Mojolicious-8.22 next generation web framework for Perl
p5-Net-DNS-1.23     perl interface to the domain name system
p5-Net-SSLeay-1.88  perl module for using OpenSSL
p5-Time-TimeDate-2.30 library for parsing and formatting dates and times
p5-common-sense-3.74 perl common defaults with lower memory usage
pango-1.44.7p0      library for layout and rendering of text
pcre-8.41p2         perl-compatible regular expression library
pcre2-10.34         perl-compatible regular expression library, version 2
png-1.6.37          library for manipulating PNG images
polkit-0.116p1      framework for granting privileged operations to users
popt-1.16p2         getopt(3)-like library with a number of enhancements
pstree-2.39p0       list processes as a tree
py3-dnspython-1.16.0p2 DNS toolkit for Python
py3-pip-19.1.1      tool for installing Python packages
py3-setuptools-41.6.0v0 simplified packaging system for Python modules
python-2.7.18p0     interpreted object-oriented programming language
python-3.7.7        interpreted object-oriented programming language
quirks-3.325        exceptions to pkg_add rules
rebar19-2.6.2p0     Erlang build tool
recode-3.6p12       convert files between character sets and usages
rhash-1.3.9         utility and library for computing hash sums
rlwrap-0.43p0       generic readline wrapper for various programs
rsync-3.1.3p0       mirroring/synchronization over low bandwidth links
rxvt-unicode-9.22p10 clone of rxvt with Unicode and Xft support
sdl-1.2.15p10       cross-platform multimedia library
sdl2-2.0.12         cross-platform multimedia library
shared-mime-info-1.15 shared mime database for desktops
speex-1.2.0         patent-free speech codec
speexdsp-1.2.0      speech processing DSP library
spidermonkey-60.9.0v1 C/C++ Mozilla's JavaScript engine implementation
sqlite3-3.31.1p0    embedded SQL implementation
startup-notification-0.12p6 library for tracking application startup
sudo-1.8.31         execute a command as another user
tiff-4.1.0          tools and library routines for working with TIFF images
tree-0.62           print ascii formatted tree of a directory structure
unzip-6.0p13        extract, list & test files in a ZIP archive
vala-0.46.9         compiler for the GObject type system
vim-8.2.534-no_x11  vi clone, many additional features
vmm-firmware-1.11.0p2 firmware binary images for vmm(4) driver
wget-1.20.3p3       retrieve files from the web via HTTP, HTTPS and FTP
windowmaker-0.95.8p4 window manager that emulates NEXTSTEP(tm)
wmclockmon-0.8.1p0  WindowMaker dock.app clock
wmnetload-1.3p4     wm-dockapp; simple network interface monitoring tool
x264-20191220p0     free H.264/MPEG-4 AVC encoder
x265-3.3p0          free H.265/HEVC encoder
xdg-utils-1.1.3p1   utilities to assist desktop integration tasks
xvidcore-1.3.5p0    ISO MPEG-4 compliant video codec
xz-5.2.5            LZMA compression and decompression tools
zip-3.0p1           create/update ZIP files compatible with PKZip(tm)
zstd-1.4.4p1        zstandard fast real-time compression algorithm
```

OpenBSD 6.7 is the ~~latest~~ stable release of [OpenBSD](https://openbsd.org).
OpenBSD 6.7 ~~has been upgraded~~ to the [6.8](/data/blog/pkg_info/68) release.
